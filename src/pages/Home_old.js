...
let this_interval = null, controlVisibleTimer, is_playing = false, is_thumbnail = true, md_content_from_api_client = '', markdownURL = '', style_text = '', styles = types.INITIAL_STYLES;
...
export default function Home(props) {
  ...
  useEffect(() => {
    if (props.csslink !== undefined && props.csslink !== '') setCsslink(props.csslink);
  }, [props.csslink]);

  useEffect(() => {
    if (props.mode !== undefined && props.mode !== '') setMode(props.mode);
  }, [props.mode]);
  ...
  const handleInterval = useCallback(() => {
    if (this_interval !== null) clearInterval(this_interval);
