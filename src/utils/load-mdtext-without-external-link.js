...
// function groupTexts(fab_canvas, id) {
//     var objs = [];
//     objs = fab_canvas._objects.filter(obj => obj.id === id).filter(function (obj) {
//         return obj;
//     });

//     var alltogetherObj = new fabric.Group(objs, { id: id, });

//     fab_canvas._objects.filter(obj => obj.id === id).forEach(function (obj) {
//         fab_canvas.remove(obj);
//     });

//     fab_canvas.add(alltogetherObj);
//     fab_canvas.renderAll();
// }

const rotateScene = (is_animation, fab_canvas, fab_video, fab_md_scene, fab_icon, fab_layout_white, playerw, currentTime, arr_script, playerscale, videowidth, videoheight, titlesize, contentsize, styles, is_right, is_videoend) => {
        ...
        _img.onload = function () {
            ...
            if (is_videoend) {
                ...
                gsap.to('#animationImg', {
                    ...
                    duration: types.ROTATE_ANIMATION_DURATION,
                    onStart: function () {
                        _loadMdText(false, fab_canvas, fab_video, fab_md_scene, fab_icon, fab_layout_white, playerw, currentTime, arr_script, playerscale, videowidth, videoheight, titlesize, contentsize, styles);
                    },
                    onComplete: function () {
...
// const _resetCanvasTextScene = (fab_canvas) => {
//     fab_canvas.getObjects().forEach((obj) => {
//         if (
//             obj.id.indexOf("animation-") > -1 ||
//             obj.id.indexOf("title-") > -1 ||
...
const checkSpecialTags = (t_font_style, t_font_weight, t_linethrough, words, ii) => {
    ...
    return [t_font_style, t_font_weight, t_linethrough, ii];
}


// let textanimated = true;
const _loadMdText = (is_animation, fab_canvas, fab_video, fab_md_scene, fab_icon, fab_layout_white, playerw, currentTime, arr_script, playerscale, videowidth, videoheight, titlesize, contentsize, styles) => {
    removeObjectsFromCanvas(fab_canvas, arr_script, currentTime, false)
    ...
    arr_script.filter(line =>
        ...
        if (fab_canvas.getObjects().filter((obj) => obj.id === id).length === 0) {
            ...
            if (isTitle) {
                handleScene(currentTime, fab_video, fab_md_scene, fab_icon, fab_layout_white, playerscale, videowidth, videoheight, line.slide, line.isLeft);
                ...
                if (initial_top + _textheight * texts.length > fab_md_scene.get("top") + fab_md_scene.get("height")) {
                    ...
                    // layout-md
                    // _resetCanvasTextScene(fab_canvas);
                }
            ...
            if ((isTitle || isSubTitle || isText) && currentTime !== 0) {
                ...
                let text_length = texts.join(' ').length;
                ...
                for (let j = 0; j < lines.length; j++) {
                    ...
                    if (is_past_text || words.length + curr_text_length + ani_chars <= text_length) {
                        ...
                        for (let ii = 0; ii <= words.length; ii++) {
                            ...
                            [t_font_style, t_font_weight, t_linethrough, ii] = checkSpecialTags(t_font_style, t_font_weight, t_linethrough, words, ii);

                            if (ii > initial_ii || ii >= words.length) {
                                // console.log('words.substring(ind, initial_ii)', words.substring(ind, initial_ii))
                                const left_pos = textobj === '' ? lPos : textobj.get("left") + textobj.get("width");
                                textobj = new fabric.Text(words.substring(ind, initial_ii), {
                                    ...config_text,
                                    left: left_pos,
                                    top: top_pos,
                                    scaleX: 1,
                        ...
                        for (let ii = 0; ii < words.length; ii++) {
                            [font_style, font_weight, linethrough, ii] = checkSpecialTags(font_style, font_weight, linethrough, words, ii);

                            let word = words[ii];
                            if (ii < words.length) {
                                if (is_past_text || ii + curr_text_length + ani_chars <= text_length) {
                                    const left_pos = textobj === '' ? lPos : textobj.get("left") + textobj.get("width");
                                    ...
                                    // is_timeline = false;
                                } else if (ii + curr_text_length + ani_chars > text_length) {
                                    let left_pos = textobj === '' ? lPos : textobj.get("left") + textobj.get("width");
                                    ...
                                    textobj = new fabric.Text(word, {
                                        ...
                                        scaleX: 5,
                                        scaleY: 5,
                                        fill: "rgba(255,255,255,0)", // isTitle ? "rgba(255,183,20,0)" : "rgba(255,255,255,0)", //ffb714
                                        fontStyle: font_style,
                                        fontWeight: font_weight,
                                        linethrough: linethrough
                                    });
                                    // console.log('left_pos2', left_pos);
                                    ...
                                    gsap.to(textobj, {
                                        ...
                                        scaleY: 1,
                                        fill: spancolor,
                                        left: left_pos + types.DIFFERENCE_LEFT,
                                        top: top_pos,
                                        delay: (ii + curr_text_length + ani_chars - text_length) / text_length,
                                        duration: 5 / 12,
                                        onComplete: function () {
                                            // if (j === lines.length - 1 && ii === words.length - 1) {
                                            //     groupTexts(fab_canvas, id);
                                            //     textanimated = true;
                                            // } else
                                            //     textanimated = false;
                                        }
    ...
    if (!isExistTimestamp) {
        handleScene(currentTime, fab_video, fab_md_scene, fab_icon, fab_layout_white, playerscale, videowidth, videoheight, types.SLIDETYPES.PRESENTERONLY, false);
    }
    // console.log('arr_script', arr_script);
    return arr_script;
};
...
let rotated = true;
export const loadMdText = (is_animation, fab_canvas, fab_video, fab_md_scene, fab_icon, fab_layout_white, playerw, currentTime, arr_script, playerscale, videowidth, videoheight, titlesize, contentsize, styles) => {
    ...
    if (is_animation && rotated) {
        ...
        if (filter_arr.length > 0) {
            rotated = false;
            // console.time("rotate");
            rotateScene(is_animation, fab_canvas, fab_video, fab_md_scene, fab_icon, fab_layout_white, playerw, currentTime, arr_script, playerscale, videowidth, videoheight, titlesize, contentsize, styles, !filter_arr[0].isLeft, filter_arr[0].slide === types.SLIDETYPES.VIDEOEND);
    ...
    if (rotated || currentTime === 0) {
        ...
        _loadMdText(is_animation, fab_canvas, fab_video, fab_md_scene, fab_icon, fab_layout_white, playerw, currentTime, arr_script, playerscale, videowidth, videoheight, titlesize, contentsize, styles);
